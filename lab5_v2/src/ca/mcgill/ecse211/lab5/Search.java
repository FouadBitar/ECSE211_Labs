...
public class Search extends Thread{
	...
	public static double[][] createWayPoints(int lx, int ly, int ux, int uy) {
		...
		waypoints[0][0] = lx * Lab5.SQUARE_SIZE;
		waypoints[0][1] = ly * Lab5.SQUARE_SIZE + (Lab5.SQUARE_SIZE/2);
		...
		for(int y = ly; y < uy + 1; y += 2) {
			if (east) {
				for(int x = lx + 1; x < ux + 1; x++, j++) {
					waypoints[j][0] = x * Lab5.SQUARE_SIZE - 10;
					waypoints[j][1] = y * Lab5.SQUARE_SIZE + (Lab5.SQUARE_SIZE/2) ;
				}
				east = false;
			} else {
